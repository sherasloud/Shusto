...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async () => {
    ...
    try {
      ...
      try {
        ...
        // Async write/update to firestore
        setDoc(doc(db, 'users', fbUser.uid), immediateProfile, { merge: true }).catch(e => {
  ...
  const demoLogin = (demoRole: 'user' | 'admin' | 'doctor' | 'pharmacy' | 'manager' | 'state' = 'user') => {
    setError(null);
    setLoading(true);
    const demoUserMap = {
      user: {
      ...
      manager: {
        uid: 'demo-manager-123',
        displayName: 'Demo Manager',
        email: 'manager@shusto.demo',
        photoURL: 'https://picsum.photos/seed/manager/200/200',
        role: 'manager' as const,
      },
      state: {
        uid: 'demo-state-123',
        displayName: 'Demo State Representative',
        email: 'state@shusto.demo',
        photoURL: 'https://picsum.photos/seed/state/200/200',
        role: 'state' as const,
      }
    };

    const selected = demoUserMap[demoRole] || demoUserMap.user;
    setUser(selected);
    sessionStorage.setItem('shusto_demo_user', JSON.stringify(selected));
    setLoading(false);

    // Sync demo user to Firestore db in background
    setDoc(doc(db, 'users', selected.uid), {
      ...selected,
      name: selected.displayName,
      createdAt: new Date().toISOString()
    }, { merge: true }).catch(err => console.warn("Demo user sync warning:", err));
  };
