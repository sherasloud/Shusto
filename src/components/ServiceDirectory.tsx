...
export function ServiceDirectory({ type, title, description }: ServiceDirectoryProps) {
  const { user } = useAuth();
  const [activeView, setActiveView] = useState<'centers' | 'services' | 'ambulance_form'>(
    type === 'ambulance' ? 'ambulance_form' : (type === 'nursing' || type === 'nutritionist' ? 'centers' : 'services')
  );
  ...
  useEffect(() => {
    ...
    if (type === 'ambulance' || type === 'nursing' || type === 'nutritionist') {
    ...
    const collectionName = type === 'pharmacy' ? 'pharmacies' : 
                         ...
                         type === 'nursing' ? 'nursings' : 
                         type === 'nutritionist' ? 'nutritionists' : 'ambulances';
    
    // Still fetch providers for context/mapping if needed, but primary view is services
    let qProviders = query(collection(db, collectionName), limit(100));
    ...
    if (type === 'pharmacy' || type === 'hospital' || type === 'nursing' || type === 'nutritionist') {
      const cacheKey = `cached_posts_${type}`;
    ...
    // Fetch Global Services if Lab, Physio, Nursing or Hospital
    if (type === 'lab' || type === 'physio' || type === 'nursing' || type === 'hospital' || type === 'nutritionist') {
      const cacheKey = `cached_global_${type}`;
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        try { 
          const parsed = JSON.parse(cached);
      ...
      setLoading(true);
      const globalColl = type === 'lab' ? 'labTests' : 
                         type === 'physio' ? 'physioServices' : 
                         type === 'nursing' ? 'nursingServices' : 
                         type === 'nutritionist' ? 'nutritionistServices' : 'hospitalServices';
      ...
      getDocs(qGlobal).then((snapshot) => {
        ...
        localStorage.setItem(`cached_global_${type}`, JSON.stringify(services));
        setLoading(false);
      }).catch((error) => {
  ...
  const handleBook = async (post?: Post | any, explicitProvider?: ServiceProvider | null) => {
    ...
    try {
      ...
      const finalPrice = isTestAccount ? 0 : price;
  ...
  return (
      ...
      ) : activeView === 'services' && (type === 'lab' || type === 'physio' || type === 'nursing' || type === 'hospital') ? (
                ...
                <div className="p-8 flex-1 flex flex-col justify-between">
                  ...
                  <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between gap-4">
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-baseline gap-1">
                        <span className="text-3xl font-black text-slate-900">৳{isTestAccount ? 0 : service.price}</span>
                      </div>
                    </div>
                    <button 
                    ...
                    <div className="p-6 flex-1 flex flex-col justify-between">
                      ...
                      <div className="mt-6 pt-6 border-t border-slate-50 flex items-end justify-between gap-4">
                        <div>
                          ...
                          <p className="text-2xl font-black text-slate-900 leading-none">৳{isTestAccount ? 0 : post.price}</p>
