...
import { collection, query, addDoc, where, getDocs, limit, startAfter, orderBy, QueryDocumentSnapshot, DocumentData, doc, getDoc, updateDoc, increment, runTransaction, writeBatch } from 'firebase/firestore';
  ...
  const fetchMedicines = async (isLoadMore = false) => {
    ...
    try {
      ...
      if (searchQuery) {
        ...
        // Use a larger limit for the "All" category to show more items at once
        q = query(
          collection(db, 'medicines'),
          limit(currentLimit)
        );
      ...
      if (newMeds.length > 0 && !searchQuery) {
        localStorage.setItem('cached_medicines', JSON.stringify(newMeds));
      }
    } catch (error: any) {
