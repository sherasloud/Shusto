...
export function DoctorDashboard() {
  ...
  const handleAddSlot = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) return;
    const selectedTime = customSlotTime.trim() || newSlotTime;
    if (!selectedTime) {
      alert("দয়া করে সময় নির্ধারণ করুন।");
      return;
    }
    ...
    try {
      const slotData = {
        ...
        maxPatients: Number(maxPatientsInput) || 1,
        bookedCount: 0,
        status: 'available',
        createdAt: new Date().toISOString()
      };

      await addDoc(collection(db, 'doctor_slots'), slotData);
      addToast("অ্যাপয়েন্টমেন্ট সময় সফলভাবে যোগ করা হয়েছে! ✔️");
      setShowAddSlotModal(false);
      setCustomSlotTime('');
    } catch (err) {
      console.error("Error adding slot:", err);
      alert("সময় যোগ করতে সমস্যা হয়েছে।");
    } finally {
      setAddingSlot(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {showAddSlotModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-[32px] p-8 shadow-2xl border border-slate-100 animate-in zoom-in duration-200">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                <Clock className="text-sky-500" size={20} />
                নতুন অ্যাপয়েন্টমেন্ট সময় তৈরি করুন
              </h2>
              <button 
            ...
            <form onSubmit={handleAddSlot} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1">তারিখ (Date)</label>
                <input 
                  type="date"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1">প্রিসেট সময় নির্বাচন (Preset Time)</label>
                <select
                  value={newSlotTime}
                  onChange={(e) => { setNewSlotTime(e.target.value); setCustomSlotTime(''); }}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500/20"
                >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1">কাস্টম সময়সূচী (অথবা কাস্টম লিখুন)</label>
                <input 
                  type="text"
                  placeholder="যেমন: 07:30 PM - 08:00 PM"
                  value={customSlotTime}
                  onChange={(e) => setCustomSlotTime(e.target.value)}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500/20 text-sm"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1">সর্বোচ্চ রোগী সংখ্যা (Max Patients)</label>
                <input 
                  type="number"
                  min="1"
                  max="50"
                  value={maxPatientsInput}
                  onChange={(e) => setMaxPatientsInput(Number(e.target.value))}
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-bold focus:outline-none focus:ring-2 focus:ring-sky-500/20"
                  required
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={addingSlot}
                  className="flex-1 py-3.5 bg-sky-500 text-white font-bold rounded-2xl hover:bg-sky-600 transition-all text-sm shadow-lg shadow-sky-500/20 disabled:opacity-50 flex items-center justify-center gap-1"
                >
                  {addingSlot ? 'সংরক্ষণ হচ্ছে...' : 'সময় সংরক্ষণ করুন ✔️'}
                </button>
              </div>
            </form>
          </div>
